...
//Modified by Pete, added instance ID "i"
func (instance *pbftCore) committed(digest string, v uint64, n uint64) bool {
	...
	if cert == nil {
        ...
        count := [4]uint64{0,0,0,0}

	counter := 0
	for _, p := range cert.commit {
		count[counter] = p.ReplicaId
		if p.View == v && p.SequenceNumber == n {
			quorum++
			counter++
		} else {
		 logger.Errorf("Sequence or view was not the same for %d, origView = %d vs newView = %d, origSeq = %d vs newSeq = %d",
			 p.ReplicaId, p.View, v, p.SequenceNumber, n)
		}
	}

	logger.Debugf("Petes Custom Replica %d commit count for view=%d/seqNo=%d: %d and the digest is: %s",
		instance.id, v, n, quorum, digest)

		check := [4]uint64{0, 1, 2, 3}
		results := [4]uint64{0, 0, 0, 0}
		for j:=0; j<4; j++ {
			for k:=0; k<4; k++ {
				if count[k] == check[j] {
					results[j] = 1
		...
		for i:=0; i < 4; i++ {
			if results[i] == 0 && instance.id == 0{
				logger.Errorf(" Passed Byzantine but... Missing replica %d in the byzantine: rep0: %d, rep1: %d, rep2: %d, rep3: %d", i, results[0], results[1], results[2], results[3])
				//logger.Errorf(" Passed Byzantine but... Missing replica %d in the byzantine: rep0: %d, rep1: %d, rep2: %d, rep3: %d", i, count[0], count[1], count[2], count[3])
				break
			}
		}
...
func (instance *pbftCore) recvPrePrepare(preprep *PrePrepare) error {
	...
	if preprep.SequenceNumber > instance.viewChangeSeqNo {
		...
		logger.Errorf("Conflict in message digest %d and %d between %d and %d", instance.viewChangeSeqNo, preprep.SequenceNumber, instance.id, preprep.ReplicaId)
		instance.sendViewChange()
		return nil
	}
	...
	if cert.digest != "" && cert.digest != preprep.BatchDigest {
		...
		logger.Errorf("Conflict in message digest %s and %s between %d and %d", cert.digest, preprep.BatchDigest, instance.id, preprep.ReplicaId)
		instance.sendViewChange()
		return nil
	}
	...
	if instance.primary(instance.view) != instance.id && instance.prePrepared(preprep.BatchDigest, preprep.View, preprep.SequenceNumber) && !cert.sentPrepare {
		...
		if instance.byzantine == true {
			prep.BatchDigest = "failCity"
		}
...
func (instance *pbftCore) maybeSendCommit(digest string, v uint64, n uint64) error {
	...
	if instance.prepared(digest, v, n) && !cert.sentCommit {
		logger.Debugf("Replica %d broadcasting commit for view=%d/seqNo=%d and %s",
			instance.id, v, n, digest)
		commit := &Commit{
			View:           v,
			SequenceNumber: n,
			BatchDigest:    digest,
...
func (instance *pbftCore) recvCommit(commit *Commit) error {
	logger.Debugf("Petes Replica %d received commit from replica %d for view=%d/seqNo=%d",
	...
	if instance.committed(commit.BatchDigest, commit.View, commit.SequenceNumber) {
		logger.Debugf("^^^ Pete recognizes that consensus is found cert commit count")
	  ...
	  logger.Debugf("!!! Pete recognizes that there is not consensus")
	}
